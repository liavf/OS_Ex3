...
void *main_thread_func(void *arg) {
    ...
    std::cout << "[THREAD] Main thread " << std::this_thread::get_id() << " is exiting." << std::endl;
    return nullptr;
}
...
void *thread_func(void *arg) {
    ...
    thread_reduce(context);
    std::cout << "[THREAD] Worker thread " << std::this_thread::get_id() << " is exiting." << std::endl;
...
void emit2 (K2* key, V2* value, void* context) {
    std::cout << "emit 2 happened" << std::endl;
...
void emit3 (K3* key, V3* value, void* context) {
    std::cout << "emit 3 happened" << std::endl;
    JobContext *job_context = static_cast<JobContext *>(context);
...
void waitForJob(JobHandle job) {
    ...
    for (std::thread& t : context->all_threads) {
        if (t.joinable()) {
            try {
                t.join();
                std::cout << "[WAIT] Joined thread " << t.get_id() << std::endl;
            } catch (const std::system_error& e) {
    ...
    std::cout << "[WAIT] All threads joined." << std::endl;
...
//    move_to_reduce(jobContext);
    std::cout << "[REDUCE] Thread " << std::this_thread::get_id() << " entered reduce." << std::endl;
    move_to_next_phase(jobContext, REDUCE_STAGE);
    while (true) {
        std::cout << "[REDUCE] Thread " << std::this_thread::get_id() << " waiting to acquire reduce_mutex." << std::endl;
        IntermediateVec curr_vec;
        jobContext->reduce_mutex.lock();
        std::cout << "[REDUCE] Thread " << std::this_thread::get_id()
                  << " acquired mutex. shuff_int_vec size: "
                  << jobContext->shuff_int_vec.size() << std::endl;
        ...
        std::cout << "[REDUCE] Thread " << std::this_thread::get_id()
                  << " popped a group with " << curr_vec.size() << " pairs." << std::endl;
        jobContext->reduce_mutex.unlock();
...
//        jobContext->atomic_progress++;
    ...
    std::cout << "[REDUCE] Thread " << std::this_thread::get_id() << " finished reducing." << std::endl;
}
...
void move_to_next_phase(JobContext *jobContext, stage_t next_stage) {
    ...
    if (next_stage == MAP_STAGE) {
        jobContext->total_progress = jobContext->input_vec->size();
    }
    else if (next_stage == SHUFFLE_STAGE) {
        jobContext->total_progress = jobContext->atomic_inter_count;
    }
    else if (next_stage == REDUCE_STAGE) {
        int reduce_total = 0;
        for (const auto& group : jobContext->shuff_int_vec) {
            reduce_total += group.size();
        }
        jobContext->total_progress = reduce_total;
    }
